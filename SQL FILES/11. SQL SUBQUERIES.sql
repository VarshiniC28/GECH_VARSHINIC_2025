...
-- FIND NAMES MARKS OF ALL STUDENTS WHOSE ROLL NO IS EVEN : NORMAL QUERY
SELECT NAME, MARKS FROM STUDENT
WHERE ROLLNO % 2 = 0;
-- -------------------------
...
SELECT (SELECT NAME FROM STUDENT WHERE MARKS > (SELECT AVG(MARKS) AS AVG_MARKS FROM STUDENT)); -- Getting error
